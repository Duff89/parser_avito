...
class Window(customtkinter.CTk):
    def __init__(self):
        ...
        self.checkbox_frame.grid(
            row=11, column=0, columnspan=2, pady=10, sticky="s")
    ...
    def main_windows_init(self):
        ...
        self.token_entry = customtkinter.CTkEntry(
            self, width=self.width_entry_field, placeholder_text="Введите токен вашего Telegram бота")
        ...
        self.chat_id_entry = customtkinter.CTkEntry(
            self, width=self.width_entry_field, placeholder_text="Введите ID чата вашего диалога в Telegram")
        ...
        self.ads_label = customtkinter.CTkLabel(
            self, text="Количество страниц:")
        self.ads_label.grid(row=3, column=0, padx=10, pady=5, sticky="w")
        self.ads_entry = customtkinter.CTkEntry(
            self, width=self.width_entry_field, placeholder_text="Сколько страниц проверять каждый раз")
        ...
        self.freq_entry = customtkinter.CTkEntry(
            self, width=self.width_entry_field, placeholder_text="Пауза между повторами. В минутах")
        ...
        self.url_entry = customtkinter.CTkEntry(
            self, width=self.width_entry_field, placeholder_text="Адрес с которого нужно начинать")
        ...
        self.min_price_label = customtkinter.CTkLabel(
            self, text="Минимальная цена:")
        self.min_price_label.grid(row=6, column=0, padx=10, pady=5, sticky="w")
        self.min_price_entry = customtkinter.CTkEntry(
            self, width=self.width_entry_field, placeholder_text="Цена больше либо равна введенному значению")
        ...
        self.max_price_label = customtkinter.CTkLabel(
            self, text="Максимальная цена:")
        self.max_price_label.grid(row=7, column=0, padx=10, pady=5, sticky="w")
        self.max_price_entry = customtkinter.CTkEntry(
            self, width=self.width_entry_field, placeholder_text="Цена меньше либо равна введенному значению")
    ...
    def telegram_log_test(self):
        ...
        if all([token, chat_id]):
            ...
            return None
        logger.info(
            "Должны быть заполнены поля ТОКЕН TELEGRAM и CHAT ID TELEGRAM")
    ...
    def start_scraping(self):
        ...
        if not url:
            logger.info(
                "Внимание! URL - обязательный параметр. Пример ссылки:")
            logger.info(
                "https://www.avito.ru/moskva/remont_i_stroitelstvo/sadovaya_tehnika-ASgBAgICAURYnAI")
            return
        """Прячем кнопку старт"""
        self.is_run = True
        ...
        self.stop_button = customtkinter.CTkButton(
            self, text="Стоп", command=self.stop_scraping)
    ...
    def stop_scraping(self):
        ...
        self.stop_button.configure(
            text='Останавливаюсь', state='disabled', row=9, column=0, padx=5, pady=5, sticky="ew")
        self.update()
    ...
    def set_up(self):
        ...
        self.config.read("settings.ini", 'UTF-8')  # читаем конфиг
    ...
    def save_config(self):
        ...
        self.config["Avito"]["URL"] = str(
            self.url_entry.get()).replace('%', '%%')  # bugfix
    ...
    def logger_tg(self):
        ...
        if token and chat_id:
            ...
            return None
        logger.info(
            "Данные для отправки в telegram не заполнены. Результат будет сохранен в файл и выведен здесь")

    def logger_widget_init(self):
        """Инициализация логирования в widget"""
        self.log_widget = customtkinter.CTkTextbox(
            self, wrap="word", width=650, height=300, text_color="#00ff26")
        self.log_widget.grid(row=10, padx=5, pady=(10, 0),
                             column=0, columnspan=2)
        logger.add(self.logger_text_widget,
...
class FeedbackFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        link_label.bind(
            "<Button-1>", lambda e: webbrowser.open_new("https://github.com/Duff89/parser_avito"))

        link_label = customtkinter.CTkLabel(self, text="Поддержать развитие проекта",
                                            text_color="grey60", cursor="hand2")
        link_label.grid(column=1, row=2, padx=10)
        link_label.bind(
            "<Button-1>", lambda e: webbrowser.open_new("https://yoomoney.ru/to/410014382689862"))
