...
PROXY_PANEL_HELP = (
    ...
    "3️) API-ключ сервиса spfa.pro + серверный прокси от mobileproxy ⭐ Рекомендуется\n"
    ...
    "• +50 ₽ на баланс после подтверждения почты на сайте spfa.pro\n"
)
...
SPFA_PROXY_REQUIRED = (
    "Подойдёт даже серверный "
    "(статический) прокси, но при его использовании установите большие паузы между "
    "запросами и повторными проверками."
)
PROXY_LINK = "https://mobileproxy.rent/user.html?buyproxy&coupons_code=eMy-r4y-FZE-kMu"
SPFA_LINK = "https://spfa.pro/"
...
PARSE_PHONE_HELP = ("Будут дополнительно собираться временные номера телефонов. Для это нужен api ключ сайта spfa.pro, "
                    "его нужно ввести в разделе <Прокси и обход блокировок>. spfa берет плату за эту услугу")
NEED_TO_INSERT_API_KEY = ("Для парсинга телефонов нужен api ключ сайта spfa.pro, "
...
BLOCK_THRESHOLD_HELP = ("Через столько плохих запросов будет срабатывать триггер, он зависит от настроек. "
                        "При использование spfa.pro для cookies - будет попытка разблокировки cookies,"
