...
#### Версия 3.2.20 — 9 августа 2026 года

Парсинг через SPFA снова работает и теперь использует мобильный API Avito.

- Добавлена настройка `purchase_cooldown` — минимальный интервал между повторными покупками cookies. Она помогает защитить баланс при сбоях парсинга или SPFA.
- При первом запуске обычные ссылки Avito автоматически преобразуются в формат мобильного API.
- Ограничение преобразования: не более двух ссылок в минуту.
- Версия протестирована с мобильным прокси и SPFA.

> [!IMPORTANT]
> Для создания персональных cookies используемый прокси передаётся сервису SPFA. По заявлению сервиса, прокси не сохраняются и не используются для других целей. Учитывайте это при принятии решения об использовании интеграции.

> [!WARNING]
> Версия находится в стадии дополнительного тестирования. Если вы обнаружили ошибку, создайте issue и приложите логи без приватных данных.


### Важные нюансы
- Cookies создаются персонально для Вас - поэтому Ваш прокси передается spfa в запросах.
Они уверяют, что не сохраняют их и дальше не используют, верить или нет - дело каждого, но предупредить я должен
